...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    public bool  enableCameraControl = true, lockAndHideMouse = false, autoGenerateCrosshair = true, showCrosshairIn3rdPerson = false, drawPrimitiveUI = false;
    ...
    Readable previousInteractableMessage = null;
    ...
    void InitInteraction(){
        ...
        if(Physics.SphereCast(playerCamera.transform.position,0.25f,playerCamera.transform.forward,out h,interactRange,interactableLayer,QueryTriggerInteraction.Ignore)){
            Readable interactable = h.collider.GetComponent<Readable>();
            if(interactable != null){
                if(!ReferenceEquals(previousInteractableMessage, interactable)){
                    interactable.ShowMessage();
                    previousInteractableMessage = interactable;
                }
            }
        }
        else{
            previousInteractableMessage?.HideMessage();
            previousInteractableMessage = null;
        }
    }
    ...
    public bool TryInteract(){
        if(cameraPerspective == PerspectiveModes._1stPerson){
            ...
            if(Physics.SphereCast(playerCamera.transform.position,0.25f,playerCamera.transform.forward,out h,interactRange,interactableLayer,QueryTriggerInteraction.Ignore)){
                ...
                if(i!=null){
                    return i.Interact(GetComponent<CafeGame.Player>());
                }
            }
        }
        return false;
...
public interface IInteractable{
    bool Interact(CafeGame.Player player);
}
...
public class SuperFPEditor : Editor{
    ...
    public override void OnInspectorGUI(){
        ...
        // t.ctaMessageChannel = (BoolStringEvent_SO)EditorGUILayout.ObjectField(new GUIContent("CTA Message channel", ""),t.ctaMessageChannel,typeof(BoolStringEvent_SO),true);
